...
namespace WLD::Camera
{
	PerspectiveCamera::PerspectiveCamera()
		: m_Projection(glm::mat4(1.0f)), m_View(glm::mat4(1.0f)), m_Position(glm::vec3(0.0f)), m_Up(glm::vec3(0.0f)), m_Pos(glm::vec3(0.0f))
	{
		recalculateVP();
...
//	}
	...
	void PerspectiveCamera::setFront(const glm::vec2& front)
	{
		glm::vec3 direction(1.0f);
		direction.x = -(cos(glm::radians(front.x) / 2.0f) * cos(glm::radians(front.y) / 2.0f));
		direction.y = sin(glm::radians(front.y) / 2.0f);
