...
namespace WLD
{
	DX12Context::DX12Context(WindowProps& props)
		: GraphicsContext(props)
	{
		m_WindowProps.Flags = (SDL_WindowFlags)(SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
	}
	...
	void DX12Context::CreateDevice()
	{
		...
		WLD_SDLCheckError(SDL_GetWindowWMInfo(m_WindowProps.SDLWindow, &m_wmInfo));
...

		{
			...
			WLD_CORE_ASSERT(!dxgiFactory->CreateSwapChainForHwnd(m_pd3dCommandQueue, m_wmInfo.info.win.window, &sd, nullptr, nullptr, &swapChain1), "Failed to create swap chain!");
	...
	void DX12Context::ImGuiInit()
	{
		ImGui_ImplSDL2_InitForD3D(m_WindowProps.SDLWindow);
		ImGui_ImplDX12_Init(m_pd3dDevice, s_NumBackBuffers, DXGI_FORMAT_R8G8B8A8_UNORM, m_pd3dSrvDescHeap, m_pd3dSrvDescHeap->GetCPUDescriptorHandleForHeapStart(), m_pd3dSrvDescHeap->GetGPUDescriptorHandleForHeapStart());
	}

	void DX12Context::ImGuiBegin()
	{
		ImGui_ImplDX12_NewFrame();
	}

	void DX12Context::ImGuiEnd()
	{
		ImGui_ImplDX12_RenderDrawData(ImGui::GetDrawData(), m_pd3dCommandList);
	}

	void DX12Context::ImGuiShutdown()
	{
		ImGui_ImplDX12_Shutdown();
	}

	void DX12Context::OnWindowResize()
	{
	}
